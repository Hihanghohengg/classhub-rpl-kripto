...
function getSortedDeadlineTasks(tasks, getStatusByTaskId) {
  return tasks
    .filter((task) => task.deadline)
    .filter((task) => getStatusByTaskId(task.id) !== 'done')
    .sort((a, b) => {
...
export default function DashboardPage() {
  ...
  const myProgressByTask = useMemo(() => {
    const map = {};

    progress.forEach((item) => {
      if (item.user_id === profile?.id) {
        map[item.assignment_id] = item.status;
      }
    });
  ...
  const getMyStatus = (taskId) => {
    return myProgressByTask[taskId] || 'not_started';
  };

  const todayTasks = tasks.filter(
    (item) =>
      item.deadline?.slice(0, 10) === today && getMyStatus(item.id) !== 'done'
  );

  const todayEvents = events.filter((item) => item.event_date === today);
  const deadlineTasks = getSortedDeadlineTasks(tasks, getMyStatus);
        ...
        {selected && selected.source === 'assignment' && (
          <div className="space-y-3 text-sm text-slate-700 dark:text-slate-200">
            ...
            <p>
              ...
              {getStatusLabel(getMyStatus(selected.id))}
            </p>
